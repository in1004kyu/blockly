...
//      var category = Blockly.createElement('div', 'grepp_category');
//      var palette = Blockly.createElement('div', 'grepp_palette');
    var code = Blockly.createElement('div', 'grepp_code');
...
//      container.appendChild(palette);
    ...
    //Create SVG dom
    //var svg_palette = Blockly.createDom_(code, options);
    var svg = Blockly.createDom_(code, options);
  } else {
    var svg = Blockly.createDom_(container, options);
  }
  //insert palette into svg_palette
...
Blockly.parseOptions_ = function(options) {
  ...
  var scroll_auto = options['scroll_auto'];
  if (scroll_auto === undefined) {
    scroll_auto = true;
  }
  ...
  if (zoom['maxScale'] === undefined) {
    zoomOptions.maxScale = 2.985984;
  } else {
    zoomOptions.maxScale = parseFloat(zoom['maxScale']);
  }
  if (zoom['minScale'] === undefined) {
    zoomOptions.minScale = 0.33489797;
  } else {
  ...
  var assetsLocation = options['rails'];
  if (assetsLocation === undefined) {
    assetsLocation = false;
  }
...
Blockly.createDom_ = function(container, options) {
  ...
  */
  ...
  /*
      <filter id="blendFilter" filterUnits="userSpaceOnUse">
      <feFlood result="flood" flood-color="#7cbb33"/>
      <feComposite result="composite" operator="in" in2="SourceGraphic"/>
      <feBlend result="blend" in2="SourceGraphic"/>
    </filter>
    <filter id="blendFilter_red" filterUnits="userSpaceOnUse">
      <feFlood result="flood" flood-color="#ee6c52"/>
      <feComposite result="composite" operator="in" in2="SourceGraphic"/>
      <feBlend result="blend" in2="SourceGraphic"/>
    </filter>
  */
  var blendFilter = Blockly.createSvgElement('filter',
    {'id': 'blendFilter'}, defs);
  Blockly.createSvgElement('feFlood', {'result': 'flood', 'flood-color': '#7cbb33'}, blendFilter);
  Blockly.createSvgElement('feComposite', {'result': 'composite', 'operator': 'in', 'in2' : 'SourceGraphic'}, blendFilter);
  Blockly.createSvgElement('feBlend', {'result': 'blend', 'in2': 'SourceGraphic'}, blendFilter);
  
  blendFilter = Blockly.createSvgElement('filter',
    {'id': 'blendFilter_red'}, defs);
  Blockly.createSvgElement('feFlood', {'result': 'flood', 'flood-color': '#ee6c52'}, blendFilter);
  Blockly.createSvgElement('feComposite', {'result': 'composite', 'operator': 'in', 'in2' : 'SourceGraphic'}, blendFilter);
  Blockly.createSvgElement('feBlend', {'result': 'blend', 'in2': 'SourceGraphic'}, blendFilter);
  /*
...
Blockly.createMainWorkspace_ = function(svg, options) {
  ...
  svg.appendChild(mainWorkspace.createBackgroundDom());
  svg.appendChild(mainWorkspace.createDom('blocklyMainBackground', 'content'));
  if (options.hasCategories) {
    mainWorkspace.svgDragCanvas_ = Blockly.createSvgElement('g', 
        {'class' : 'svgDragCanvas'}, null, this);
    svg.appendChild(mainWorkspace.svgDragCanvas_);
  }
  ...
  if (!options.readOnly && !options.hasScrollbars) {
    var workspaceChanged = function() {
      if (Blockly.dragMode_ == 0) {
        ...
        var edgeLeft = metrics.viewLeft + metrics.absoluteLeft;// +261;
        var edgeRight = metrics.viewLeft + metrics.absoluteLeft;
        ...
        if (metrics.contentTop < edgeTop ||
            ...
            metrics.contentLeft + metrics.contentWidth > (options.RTL ?
                metrics.viewWidth : metrics.viewWidth + edgeRight)) {
          ...
          for (var b = 0, block; block = blocks[b]; b++) {
            ...
            /*
            for (var nextblock = block.nextConnection; 
                  nextblock.targetConnection;
                  nextblock = nextblock.targetConnection.sourceBlock_.nextConnection) {
              var height = nextblock.targetConnection.sourceBlock_.getHeightWidth();
              blockHW.height += height.height;
            }
            */
            // Bump any block that's above the top back inside.
            var overflow = edgeTop + MARGIN - blockHW.height - blockXY.y;
            //var overflow = edgeTop - blockXY.y;
            if (overflow > 0) {
            ...
            var overflow = MARGIN + edgeLeft -
                ...
                //blockXY.x - (options.RTL ? 0 : blockHW.width / 10);
            if (overflow > 0) {
              block.moveBy(overflow, 0);
            }
            // Bump any block that's off the right back inside.
            //var overflow = edgeRight + metrics.viewWidth - MARGIN - 10 -
            var overflow = edgeRight + metrics.viewWidth - MARGIN -
...
Blockly.init_ = function(mainWorkspace) {
  ...
  if (options.languageTree) {
    if (mainWorkspace.toolbox_) {
      ...
      //mainWorkspace.category_toolbox_.init(mainWorkspace);
