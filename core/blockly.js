...
goog.require('Blockly.Category_toolbox');
...
Blockly.CATE_IMG = {
  ...
  catControl: "assets/images/flow.png",
  catLoops: "assets/images/movement.png"
};
...
Blockly.onMouseMove_ = function(e) {
  ...
  if (workspace.isScrolling) {
    ...
    if (workspace.options.scroll_auto) {
      if (workspace.is_scroll_vertical) {
      ...
      if (workspace.is_scroll_horizon) {
...
Blockly.getMainWorkspaceMetrics_ = function() {
  ...
  if (this.scrollbar) {
    ...
    if (this.options.scroll_auto) {
      var topEdge = Math.min(contentY - viewHeight / 20,
                             contentY + contentHeight - viewHeight);
      var bottomEdge = Math.max(contentY + contentHeight + viewHeight / 20,
                                contentY + viewHeight);
      var leftEdge = Math.min(contentX - viewWidth / 20,
                              contentX + contentWidth - viewWidth);
      var rightEdge = Math.max(contentX + contentWidth + viewWidth / 20,
                               contentX + viewWidth);
      if (topEdge == contentY + contentHeight - viewHeight) {
        this.is_scroll_vertical = false;
        this.scrollbar.vScroll.setVisible(false);
      } else {
        this.is_scroll_vertical = true;
        this.scrollbar.vScroll.setVisible(true);
      }
      if (leftEdge == contentX + contentWidth - viewWidth) {
        this.is_scroll_horizon = false;
        this.scrollbar.hScroll.setVisible(false);
      } else {
        this.is_scroll_horizon = true;
