...
Blockly.BlockSvg = function(workspace, prototypeName, opt_id) {
  ...
  this._icon_circle = Blockly.createSvgElement('circle', {'class': 'iconOutlineCricle'},
      this.svgGroup_);

  this._icon_bottom = Blockly.createSvgElement('path', {'class': 'ic_replay'},
      this.svgGroup_);
  this._icon_rect = Blockly.createSvgElement('rect', {'class': 'iconOutlineCricle'},
      this.svgGroup_);

  this._icon_group = Blockly.createSvgElement('g', {}, this.svgGroup_);

  this._icon = Blockly.createSvgElement('path', {},
      this._icon_group);

  this._icon_circle_image = Blockly.createSvgElement('circle', {},
      this._icon_group);

  this._icon_back = Blockly.createSvgElement('path', {},
    this._icon_group);
  this._icon_text = Blockly.createSvgElement('text', {'class': 'icon_if_text'},
    this.svgGroup_);
...
Blockly.BlockSvg.prototype._icon_circle = null;

Blockly.BlockSvg.prototype._icon_rect = null;

Blockly.BlockSvg.prototype._icon = null;

Blockly.BlockSvg.prototype._icon_back = null;

Blockly.BlockSvg.prototype._icon_bottom = null;

Blockly.BlockSvg.prototype._icon_text = null;

Blockly.BlockSvg.prototype._icon_group = null;

Blockly.BlockSvg.prototype._icon_circle_image = null;

Blockly.BlockSvg.prototype._isDrag = false;
...
/**
 * Height of this block, when draw right bottom. For bottom right icon
 */
Blockly.BlockSvg.prototype.rightHeight = 0;
/**
 * Width of this block, when drwa right bottom. For bottom right icon
 */
Blockly.BlockSvg.prototype.rightWidth = 0;

Blockly.BlockSvg.prototype.offset_additional_padding = 0;
/**
...
Blockly.BlockSvg.prototype.initSvg = function() {
  ...
  if (!this.workspace.options.readOnly && !this.eventsInit_) {
    ...
    //Bug.. 
    //Blockly.bindEvent_(this.getSvgRoot(),'mouseover', thisBlock, this.addSelect);
    //Blockly.bindEvent_(this.getSvgRoot(),'mouseleave', thisBlock, this.removeselect);    

    Blockly.bindEvent_(this.getSvgRoot(),'mouseover', thisBlock, function() {
    var url = 'url(' + Blockly.Css.mediaPath_ + '/' + Blockly.Css.currentCursor_ + '.cur), auto';
        this.getSvgRoot().style.cursor = url
    });
    Blockly.bindEvent_(this.getSvgRoot(),'mouseleave', thisBlock, function(){
    var url = 'url(' + Blockly.Css.mediaPath_ + '/' + Blockly.Css.currentCursor_ + '.cur), auto';
        this.getSvgRoot().style.cursor = url
    });
    Blockly.bindEvent_(this.getSvgRoot(),'mousemove', thisBlock, function(){
    var url = 'url(' + Blockly.Css.mediaPath_ + '/' + Blockly.Css.currentCursor_ + '.cur), auto';
        this.getSvgRoot().style.cursor = url
    });
  ...
  // For tutorial focus
  if(this.focusId)
    this.svgGroup_.setAttribute('id', this.focusId);
};
...
/**
 * Block's sign Icon (circle)
 *
 */
Blockly.BlockSvg.prototype.signicon_circle = null;

/**
 * Block's sign Icon (rect)
 *
 */
Blockly.BlockSvg.prototype.signicon_rect = null;
...
Blockly.BlockSvg.terminateDrag_ = function() {
  ...
  if (Blockly.dragMode_ == 2) {
    ...
    if (selected) {
      ...
      //Set outline drag block
      var removeDom = selected.workspace.svgDragCanvas_.removeChild(selected.svgGroup_);
      selected.workspace.svgBlockCanvas_.appendChild(removeDom)
...
Blockly.BlockSvg.prototype.getRelativeToSurfaceXY = function() {
  ...
  if (element) {
    do {
      ...
      if (element == this.workspace.getDragCanvas())
        break;
...
Blockly.BlockSvg.prototype.onMouseDown_ = function(e) {
  if (this.isInFlyout) {
    this.workspace.targetWorkspace.callbackClickBlock();
    this.workspace.targetWorkspace.isDoneHighlight = true;
    if(this.workspace.targetWorkspace.toolbox_)
      this.workspace.targetWorkspace.setChangestate(Blockly.Workspace.STATE_CREATEINTOOLBOX);
    return;
  }
  this.workspace.callbackClickBlock();

  //하이라이트 기능 끄기
  this.workspace.isDoneHighlight = true;
  //For undo redo.
  if(this.workspace.getChangestate() != Blockly.Workspace.STATE_CREATE &&
  ...
  // For update block number
  if (this.workspace.callbackUpdateBlock)
    this.workspace.callbackUpdateBlock();
};
...
Blockly.BlockSvg.prototype.onMouseUp_ = function(e) {
  ...
  var flyout = document.getElementById("flyoutmain");
  var mainworkspace = document.getElementById("mainWorkspace");

  if (!this.workspace.toolbox_)
    mainworkspace.insertBefore(maincontent, flyout);
  else {
    goog.dom.insertSiblingAfter(flyout, mainworkspace);
  }
  */
  Blockly._isDrag = false;
  ...
  if (Blockly.selected && Blockly.highlightedConnection_) {
    ...
    if (this_.rendered) {
      // Check change state. For redo/undo      
      if(this_.workspace.getChangestate() == Blockly.Workspace.STATE_DISCONNECT) {
        this_.workspace.rollbackUndostack();
      } 
      this_.workspace.setChangestate(Blockly.Workspace.STATE_INIT);
    ...
    // Check change state. For redo/undo
    if(this_.workspace.getChangestate() == Blockly.Workspace.STATE_CREATE) {
        this_.workspace.rollbackUndostack();
        this_.workspace.setChangestate(Blockly.Workspace.STATE_INIT );
    }
    ...
    // Check change state. For redo/undo
    if(this_.workspace.getChangestate() != Blockly.Workspace.STATE_DISCONNECT &&
      this_.workspace.getChangestate() != Blockly.Workspace.STATE_CREATE) {
      //Change state
      this_.workspace.rollbackUndostack();
    }
    this_.workspace.setChangestate(Blockly.Workspace.STATE_INIT);
  ...
  // For update block number
  if (workspace.callbackUpdateBlock) {
...
*/
    ...
    var deleteOption = {
      text: Blockly.Msg.DELETE_X_BLOCKS,
      enabled: true,
      callback: function() {
        var workspace = block.workspace;
        block.workspace.saveWorkspace();
...
Blockly.BlockSvg.prototype.onMouseMove_ = function(e) {
  ...
  //document.getElementsByClassName("blocklyDraggable").


  Blockly.removeAllRanges();
  if(Blockly._isDrag == undefined)
    Blockly._isDrag = false;
  if(Blockly._isDrag == false && !workspace_.toolbox_) {
    /*
    var maincontent = document.getElementById("mainContent");
    var flyout = document.getElementById("flyoutmain");
    var mainworkspace = document.getElementById("mainWorkspace");
    mainworkspace.insertBefore(flyout, maincontent);
    */
    Blockly._isDrag = true;
  } else if(Blockly._isDrag == false) {
    /*
    var maincontent = document.getElementById("mainContent");
    var flyout = document.getElementById("flyoutmain");
    var mainworkspace = document.getElementById("mainWorkspace");
    goog.dom.insertSiblingAfter(mainWorkspace, flyout);
    */
    //mainworkspace.insertBefore(flyout, maincontent);
    Blockly._isDrag = true;
  }
  ...
  if (Blockly.dragMode_ == 1) {
    ...
    if (dr > Blockly.DRAG_RADIUS) {
      ...
      if (this_.parentBlock_) {
        ...
        workspace_.setChangestate(Blockly.Workspace.STATE_DISCONNECT);
      }
      this_.setDragging_(true);
      // Block into dragcanvas.
      var removeDom = this_.workspace.svgBlockCanvas_.removeChild(this_.svgGroup_);
      this_.workspace.svgDragCanvas_.appendChild(removeDom)
    }
  }
  if (Blockly.dragMode_ == 2) {
 ...
 * Select this block.  Highlight it visually.
 */
Blockly.BlockSvg.prototype.addSelectForHighligt = function(e) {
  Blockly.addClass_(/** @type {!Element} */ (this.svgGroup_),
                    'blocklySelected');
};
 ...
 * Select this block.  Highlight it visually.
 */
Blockly.BlockSvg.prototype.addSelect = function(e) {
  Blockly.addClass_(/** @type {!Element} */ (this.svgGroup_),
                    'blocklySelected');
