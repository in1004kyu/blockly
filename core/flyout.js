...
Blockly.Flyout.prototype.width_destination_ = 0;
...
Blockly.Flyout.minWidth_ = 154;

/**
 * Task ID of sliding workspace
 * @private
 */
Blockly.Flyout.prototype.slideTask_ = 0;
...
Blockly.Flyout.prototype.createDom = function() {
  ...
  */
  ...
  this.svgGroup_.setAttribute('id', 'flyoutmain');
...
Blockly.Flyout.prototype.dispose = function() {
  goog.Timer.clear(this.slideTask_);
  this.hide();
...
Blockly.Flyout.prototype.position = function() {
  ...
  var path = ['M ' + (this.RTL ? this.width_ : 0) + ',8'];
  path.push('a 8 8 0 0 1 8 -8');
  path.push('h', edgeWidth - 8);
  ...
  if(this.targetWorkspace_.toolbox_) {
    ...
    var x = this.targetWorkspace_.toolbox_.width;
  }
  else 
    var x = 0;
  ...
  // For last padding
  this.height_ = metrics.viewHeight - 5;
...
Blockly.Flyout.prototype.hide = function(option) {
  ...
  if(this.targetWorkspace_.toolbox_ && option == true) {
    goog.Timer.clear(this.slideTask_);
    this.width_destination_ = 0;
    goog.dom.insertSiblingAfter(this.targetWorkspace_.svgBackground_space, 
      this.svgGroup_);
    goog.dom.insertSiblingAfter(this.targetWorkspace_.svgGroup_, 
      this.targetWorkspace_.svgBackground_space);
    this.slideFlyout_();
  } else if (this.targetWorkspace_.toolbox_ && option == false) {
    this.width_ = 0;
...
Blockly.Flyout.prototype.slideFlyout_ = function() {
  if(this.width_destination_ == 0) {
    if(this.width_ > this.width_destination_) {
      this.width_-= 10;
      if(this.width_ <= this.width_destination_) {
        this.svgGroup_.style.display = 'none';
        this.width_ = this.width_destination_;
        goog.dom.insertSiblingAfter(this.svgGroup_, 
          this.targetWorkspace_.svgGroup_);
        goog.dom.insertSiblingAfter(this.targetWorkspace_.svgGroup_, 
          this.targetWorkspace_.svgBackground_space);

      }
      Blockly.fireUiEvent(window, 'resize');
      this.slideTask_ = goog.Timer.callOnce(this.slideFlyout_, 20, this);
    }
  } else if(this.width_destination_ > 0) {
    if(this.width_ < this.width_destination_) {
      this.width_ += 10;
      if(this.width_ >= this.width_destination_) {
        this.width_ = this.width_destination_;
 ...
 */
Blockly.Flyout.prototype.show = function(xmlList, option) {
  this.hide();
  ...
  for (var i = 0, xml; xml = xmlList[i]; i++) {
    if (xml.tagName && xml.tagName.toUpperCase() == 'BLOCK') {
      ...
      gaps.push(isNaN(gap) ? margin : gap);
    }
  }
  ...
  for (var i = 0, block; block = blocks[i]; i++) {
    ...
    // margin left edge of layout
    var x = this.RTL ? 0 : margin / this.workspace_.scale +
        Blockly.BlockSvg.TAB_WIDTH - 9;
 ...
 */
Blockly.Flyout.prototype.reflow = function(thisObject, e, option) {
  this.workspace_.scale = this.targetWorkspace_.scale;
  //Flyout is not affected by scale. So fix scale number is 1.
  this.workspace_.scale = 1;
  var margin = this.CORNER_RADIUS;
  var flyoutWidth = Blockly.Flyout.minWidth_ - 
                    (Blockly.BlockSvg.TAB_WIDTH - 4) - 
                    (margin * 1.5 + Blockly.Scrollbar.scrollbarThickness);
  var blocks = this.workspace_.getTopBlocks(false);
  for (var x = 0, block; block = blocks[x]; x++) {
    var width = block.getHeightWidth().width;
    if (block.outputConnection) {
      width -= Blockly.BlockSvg.TAB_WIDTH - 4;
    }
    flyoutWidth = Math.max(flyoutWidth, width);
  }
  flyoutWidth += (Blockly.BlockSvg.TAB_WIDTH - 4);
  ...
  if (this.width_ != flyoutWidth) {
    for (var x = 0, block; block = blocks[x]; x++) {
      ...
      if (this.RTL) {
        ...
        dx -= Blockly.BlockSvg.TAB_WIDTH - 4;
      ...
      if (block.flyoutRect_) {
        ...
        var tab = block.outputConnection ? Blockly.BlockSvg.TAB_WIDTH - 4 : 0;
    ...
    // Fire a resize event to update the flyout's scrollbar.
    if(this.targetWorkspace_.toolbox_ && option == true) {
      this.width_destination_ = flyoutWidth;
      goog.Timer.clear(this.slideTask_);
      goog.dom.insertSiblingAfter(this.targetWorkspace_.svgBackground_space, 
        this.svgGroup_);
      goog.dom.insertSiblingAfter(this.targetWorkspace_.svgGroup_, 
      ...
      Blockly.fireUiEvent(window, 'resize');      
    } else if(!this.targetWorkspace_.toolbox_) {
      this.width_ = flyoutWidth;
      Blockly.fireUiEvent(window, 'resize');
    } else if(!option) {
      return;
      ...
      console.log(this.targetWorkspace_.toolbox_);
      console.log(option);
      this.width_ = flyoutWidth;
      Blockly.fireUiEvent(window, 'resize');
    }
...
Blockly.Flyout.prototype.onMouseMoveBlock_ = function(e) {
  ...
  if (Math.sqrt(dx * dx + dy * dy) > Blockly.DRAG_RADIUS) {
    // Set changestate    
    var workspace = Blockly.Flyout.startFlyout_.getTargetWorkspace_()();
    workspace.setChangestate(Blockly.Workspace.STATE_CREATE);
...
Blockly.Flyout.prototype.getTargetWorkspace_ = function() {
...
Blockly.Flyout.prototype.createBlockFunc_ = function(originBlock) {
  ...
  return function(e) {
    ...
    if(workspace.getChangestate() == Blockly.Workspace.STATE_CREATEINTOOLBOX) {
      workspace.setChangestate(Blockly.Workspace.STATE_CREATE);
      workspace.saveWorkspace();
    }
    ...
    */
    ...
    // Scale the scroll (getSvgXY_ did not do this).
    // We do not need scale flyout's blocks
