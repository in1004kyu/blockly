...
Blockly.Toolbox.prototype.init = function() {
  ...
  this.addIcon_();
  this.position();
  tree.setSelectedItem(this.tree_.children_[0]);
  //this.flyout_.show(workspace.options.languageTree.getElementsByTagName("category")[0].getElementsByTagName("block")); 
};
    ...
    for (var i = 0, childIn; childIn = treeIn.childNodes[i]; i++) {
      ...
      switch (childIn.tagName.toUpperCase()) {
        case 'CATEGORY':
          ...
          // Set image icon
          childOut.iconUrl = Blockly.CATE_IMG[childIn.getAttribute('id')];
...
Blockly.Toolbox.prototype.addColour_ = function(opt_tree) {
  ...
  for (var i = 0, child; child = children[i]; i++) {
    ...
    if (element) {
      if (this.hasColours_) {
        var border = 'none';//'8px solid ' + (child.hexColour || '#ddd');
      } else {
        var border = 'none';
      }
      if (this.workspace_.RTL) {
...
Blockly.Toolbox.prototype.addIcon_ = function(opt_tree) {
  var tree = opt_tree || this.tree_;
  var children = tree.getChildren();
  for (var i = 0, child; child = children[i]; i++) {
    var element = child.getRowElement();
    if (element) {
      var iconUrl = child.iconUrl;
...
Blockly.Toolbox.TreeControl.prototype.setSelectedItem = function(node) {
  ...
  if (node) {
    ...
    node.getRowElement().style.backgroundColor = '#ededed';//hexColour; static color, grey.
  ...
  if (node && node.blocks && node.blocks.length) {
   // if(toolbox.flyout_.svgGroup_.style.display == "none") {
    if(toolbox.flyout_.status_open == false) {
      ...
      toolbox.flyout_.show(node.blocks, false);
