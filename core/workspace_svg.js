...
Blockly.WorkspaceSvg.prototype.is_scroll_horizon = false;

Blockly.WorkspaceSvg.prototype.is_scroll_vertical = false;

Blockly.WorkspaceSvg.prototype.createBackgroundDom = function() {
  this.svgBackground_space = Blockly.createSvgElement('g',
      {'class': 'greppBackgroundspace'}, null);
  this.svgBackground_space_svg = Blockly.createSvgElement('rect',
        {'height': '100%', 'width': '100%',
         'class': 'svgBackground_space_svg'}, this.svgBackground_space);
  this.svgBackground_space_svg.style.fill = "white";

  return this.svgBackground_space;
}
 ...
 */
Blockly.WorkspaceSvg.prototype.createDom = function(opt_backgroundClass, options) {
  /**
  ...
  if(options == 'content') {
    this.svgGroup_.setAttribute('id', 'mainWorkspace');
    this.svgBlockCanvas_.setAttribute('id', 'mainContent');
  }
  /** @type {SVGElement} */
  ...
  //Remove mouse leave, over event : do not need remove scroll bar. 03/14
  //Blockly.bindEvent_(this.svgGroup_, 'mouseleave', this, this.onMouseLeave_);
  //Blockly.bindEvent_(this.svgGroup_, 'mouseover', this, this.onMouseOver_);
  
  if (options == "content" && !this.options.hasCategories) {
    this.svgDragCanvas_ = Blockly.createSvgElement('g', 
      {'class' : 'svgDragCanvas'}, this.svgGroup_, this);
  }
  if (options == "content") {
...
//    this.category_toolbox_ = new Blockly.Category_toolbox(this);
...
Blockly.WorkspaceSvg.prototype.backgroundPosition = function() {
  this.svgBackground_space_svg.style.x = this.getMetrics().absoluteLeft;
}
...
Blockly.WorkspaceSvg.prototype.getDragCanvas = function() {
...
Blockly.WorkspaceSvg.prototype.translate = function(x, y) {
  ...
  if (this.svgDragCanvas_)
    this.svgDragCanvas_.setAttribute('transform', translation);
};
...
/**
 * 하이라이트 대상인 블록 모음, 멀티 하이라이트용
 * @type {Array}
 */
Blockly.WorkspaceSvg.prototype.listOfHighlightBlock = [];

/**
 * 하이라이트 블록들을 전부 삭제하기 - 초기화용
 */
Blockly.WorkspaceSvg.prototype.clearListOfHlb = function() {
  for (var i = 0; i < this.listOfHighlightBlock.length; i++) {
    // 이미 지워진 경우라면 패쓰
    if (this.listOfHighlightBlock[i] && 
      this.listOfHighlightBlock[i].svgGroup_) {
      this.listOfHighlightBlock[i].removeSelect();
    }
  }
  this.listOfHighlightBlock = [];
}
...
Blockly.WorkspaceSvg.prototype.highlightBlock = function(id, jobId) {
  ...
  if (block) {
    //block.select();
    // 현재 탑 블록에 속해있는 블록이라면 해당 라인의 하이라이트르 삭제하고
    // 이번 블록을 하이라이트 시킴. 
    //var topblockId = this.getTopBlockId(id);

    //if (topblockId == -1) {
    //  console.error("error!");
    //}

    if (this.listOfHighlightBlock[jobId]) {
      this.listOfHighlightBlock[jobId].removeSelect();
    }
    this.listOfHighlightBlock[jobId] = block;
    //this.listOfHighlightBlock[jobId].addSelectForHighligt();
    this.listOfHighlightBlock[jobId].addSelect();
    Blockly.fireUiEvent(this.getCanvas(), 'blocklySelectChange');
...
Blockly.WorkspaceSvg.prototype.onMouseLeave_ = function(e) {
  e.stopPropagation();

  if(this.scrollbar == null)
    return;
  this.scrollbar.hScroll.setVisible(false);
  this.scrollbar.vScroll.setVisible(false);
}
Blockly.WorkspaceSvg.prototype.onMouseOver_ = function(e) {
  ...
  if(this.scrollbar == null)
    return;
  this.scrollbar.hScroll.setVisible(true);
  this.scrollbar.vScroll.setVisible(true);
}

Blockly.WorkspaceSvg.prototype.isDoneHighlight = true;
...
Blockly.WorkspaceSvg.prototype.onMouseWheel_ = function(e) {
  ...
  */
  if (!this.options.scroll_auto) {
    ...
    var dy, y, dx, x = 0;
    // Move the scrollbars and the page will scroll automatically.
    if (this.is_scroll_vertical) {
      dy = e.wheelDeltaY || -e.deltaY;
      Blockly.mainWorkspace.startScrollY = Blockly.mainWorkspace.scrollY;
      y = Blockly.mainWorkspace.startScrollY + dy;
      y = Math.min(y, -metrics.contentTop);
      y = Math.max(y, metrics.viewHeight - metrics.contentTop -
                 metrics.contentHeight);
      Blockly.mainWorkspace.scrollbar.vScroll.set(-y - metrics.contentTop);
    } 
    if (this.is_scroll_horizon) {
      dx = e.wheelDeltaX || -e.deltaX;
      Blockly.mainWorkspace.startScrollX = Blockly.mainWorkspace.scrollX;
      x = Blockly.mainWorkspace.startScrollX + dx;
      x = Math.min(x, -metrics.contentLeft);
      x = Math.max(x, metrics.viewWidth - metrics.contentLeft -
              metrics.contentWidth);
      Blockly.mainWorkspace.scrollbar.hScroll.set(-x - metrics.contentLeft);
    }
  }
  e.stopPropagation();
  e.preventDefault();
...
Blockly.WorkspaceSvg.prototype.zoom = function(x, y, type) {
  ...
  if (this.flyout_) {
    ...
    this.flyout_.reflow(this, null ,false);
  }
};
...
Blockly.WorkspaceSvg.prototype.zoomReset = function(e) {
  ...
  if (this.scrollbar) {
    ...
    this.scrollbar.set((metrics.contentWidth - metrics.viewWidth) / 2,
        ...
        */
    this.scrollbar.set(
      (metrics.viewWidth - this.svgBlockCanvas_.getBBox().width - 20), 
      (metrics.viewHeight - this.svgBlockCanvas_.getBBox().height - 20));
  } else {
    this.translate(0, 0);
  }
