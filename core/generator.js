...
Blockly.Generator.prototype.testCode = function(workspace) {
  var code = [];
  this.init(workspace);
  var blocks = workspace.getTopBlocks(true);
  for (var i = 0; i < blocks.length; i++) {
    if(blocks[i].type == "procedures_defnoreturn" ||
      ...
      var line = this.blockToCode(blocks[i]);
      console.log(line);
      code = code.join('\n');  // Blank line between each section.
      code = this.finish(code);
      // Final scrubbing of whitespace.
...
Blockly.Generator.prototype.getCodeFromBlock = function(block, workspace) {
  if (!workspace) {
    // Backwards compatability from before there could be multiple workspaces.
    console.warn('No workspace specified in workspaceToCode call.  Guessing.');
    workspace = Blockly.getMainWorkspace();
  }
  var firstBlocks = block.getChildren();
  var code = [];
  this.init(workspace);
  var line = this.blockToCode(firstBlocks[0]);
  if (goog.isArray(line)) {
      // Value blocks return tuples of code and operator order.
      // Top-level blocks don't care about operator order.
      line = line[0];
    ...
    if (line) {
      if (firstBlocks[0].outputConnection && this.scrubNakedValue) {
        // This block is a naked value.  Ask the language's code generator if
        // it wants to append a semicolon, or something.
        line = this.scrubNakedValue(line);
      }
...
Blockly.Generator.prototype.blocksToCode = function(workspace, name) {
  if (!workspace) {
    // Backwards compatability from before there could be multiple workspaces.
    console.warn('No workspace specified in workspaceToCode call.  Guessing.');
    workspace = Blockly.getMainWorkspace();
  ...
  for (var x = 0, block; block = blocks[x]; x++) {
      if(block.type == name) {
          if(block.type == "start_block" && !block.getChildren()){
            continue;
          }
        var line = this.blockToCode(block);
        if (goog.isArray(line)) {
